...
#Region "Methods"
	...
	Public Sub ReadFile(ByVal inputPathFileName As String, ByVal aSourceEngineModel As SourceModel_Old)
		...
		Try
			...
			If inputFileStream IsNot Nothing Then
				Try
					...
					If Me.theSourceEngineModel.PhyFileHeader.solidCount > 0 Then
...
#Region "Private Methods"
	...
	Private Sub ReadSourcePhyHeader()
		Me.theSourceEngineModel.PhyFileHeader = New SourcePhyFileData()
		...
		'12 00 00 00 
		'1f de 9d 20 
		Me.theSourceEngineModel.PhyFileHeader.size = Me.theInputFileReader.ReadInt32()
		Me.theSourceEngineModel.PhyFileHeader.id = Me.theInputFileReader.ReadInt32()
		Me.theSourceEngineModel.PhyFileHeader.solidCount = Me.theInputFileReader.ReadInt32()
		Me.theSourceEngineModel.PhyFileHeader.checksum = Me.theInputFileReader.ReadInt32()

		'NOTE: If header size ever increases, this will at least skip over extra stuff.
		Me.theInputFileReader.BaseStream.Seek(Me.theSourceEngineModel.PhyFileHeader.size, SeekOrigin.Begin)
	End Sub
	...
	Private Sub ReadSourceCollisionData()
		...
		Me.theSourceEngineModel.PhyFileHeader.theSourcePhyCollisionDatas = New List(Of SourcePhyCollisionData)()
		For solidIndex As Integer = 0 To Me.theSourceEngineModel.PhyFileHeader.solidCount - 1
			...
			While Me.theInputFileReader.BaseStream.Position < vertexDataStreamPosition
				...
				If faceSection.theBoneIndex < 0 Then
					...
					Me.theSourceEngineModel.PhyFileHeader.theSourcePhyIsCollisionModel = True
				End If
			...
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyCollisionDatas.Add(collisionData)
	...
	Private Sub ReadSourcePhysCollisionModels()
		Try
			...
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyPhysCollisionModels = New List(Of SourcePhyPhysCollisionModel)()
			...
			Do
				...
				If key Is Nothing OrElse key <> "}" Then
					Exit Do
				End If
				Me.theSourceEngineModel.PhyFileHeader.theSourcePhyPhysCollisionModels.Add(aSourcePhysCollisionModel)
				thereIsAValue = True
			...
			maxCount = 0
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues = New SourcePhyPhysCollisionModel()
			For i As Integer = 0 To Me.theDampingToCountMap.Count - 1
				...
				End If
			Next
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues.theDamping = maxValue
			maxCount = 0
			For i As Integer = 0 To Me.theInertiaToCountMap.Count - 1
				...
				End If
			Next
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues.theInertia = maxValue
			maxCount = 0
			For i As Integer = 0 To Me.theRotDampingToCountMap.Count - 1
				...
				End If
			Next
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues.theRotDamping = maxValue
		Catch
			Throw
			'Finally
		End Try
	...
	Private Sub ReadSourcePhyRagdollConstraintDescs()
		Try
			...
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyRagdollConstraintDescs = New SortedList(Of Integer, SourcePhyRagdollConstraint)()
			Do
				...
				If key Is Nothing OrElse key <> "}" Then
					Exit Do
				End If
				Me.theSourceEngineModel.PhyFileHeader.theSourcePhyRagdollConstraintDescs.Add(aSourceRagdollConstraintDesc.theChildIndex, aSourceRagdollConstraintDesc)
				thereIsAValue = True
			Loop Until line Is Nothing
		Catch
		Finally
	...
	Private Sub ReadSourcePhyCollisionRules()
		Try
			...
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyCollisionPairs = New List(Of SourcePhyCollisionPair)()
			Do
				...
				While thereIsAValue
					...
					If thereIsAValue Then
						...
						ElseIf key = "selfcollisions" Then
							Me.theSourceEngineModel.PhyFileHeader.theSourcePhySelfCollides = False
						End If
					End If
				End While
				...
				If key Is Nothing OrElse key <> "}" Then
					Exit Do
				End If
				Me.theSourceEngineModel.PhyFileHeader.theSourcePhyCollisionPairs.Add(aSourcePhyCollisionPair)
				thereIsAValue = True
			Loop Until line Is Nothing
		Catch
		Finally
	...
	Private Sub ReadSourcePhyEditParamsSection()
		Try
			...
			Dim value As String = ""
			Dim tempStreamOffset As Long
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyEditParamsSection = New SourcePhyEditParamsSection()
			Do
				tempStreamOffset = Me.theInputFileReader.BaseStream.Position
				line = FileManager.ReadTextLine(Me.theInputFileReader)
				...
				While thereIsAValue
					...
					If key = "rootname" Then
						thereIsAValue = True
						If key <> value Then
							Me.theSourceEngineModel.PhyFileHeader.theSourcePhyEditParamsSection.rootName = value
							'Else
							'	Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection.rootName = ""
						End If
					ElseIf thereIsAValue Then
						If key = "concave" Then
							Me.theSourceEngineModel.PhyFileHeader.theSourcePhyEditParamsSection.concave = value
						ElseIf key = "totalmass" Then
							Me.theSourceEngineModel.PhyFileHeader.theSourcePhyEditParamsSection.totalMass = Single.Parse(value, TheApp.InternalNumberFormat)
						End If
					End If
				End While
	...
	Private Sub ReadCollisionTextSection()
		...
		Try
			...
			Me.theSourceEngineModel.PhyFileHeader.theSourcePhyCollisionText = Me.theInputFileReader.ReadChars(CInt(streamLastPosition - Me.theInputFileReader.BaseStream.Position - 1))
		Catch
		Finally
		End Try
	End Sub
