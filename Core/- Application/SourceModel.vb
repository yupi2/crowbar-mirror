...
Public Class SourceModel
	...
	Public Function GetBodyGroupSmdFileName(ByVal bodyPartIndex As Integer, ByVal modelIndex As Integer, ByVal lodIndex As Integer) As String
		...
		If (bodyPartIndex = 0 AndAlso modelIndex = 0 AndAlso lodIndex = 0) _
		 AndAlso (Me.theMdlFileHeader.theModelCommandIsUsed OrElse (Me.theMdlFileHeader.theBodyParts.Count = 1 AndAlso Me.theMdlFileHeader.theBodyParts(0).theModels.Count = 1)) _
		 Then
			bodyGroupSmdFileName = Me.theModelName
			bodyGroupSmdFileName += "_reference"
			bodyGroupSmdFileName += ".smd"
		Else
			aBodyPart = Me.theMdlFileHeader.theBodyParts(bodyPartIndex)
	...
	Public Function GetAnimationSmdRelativePath() As String
		Return Me.theModelName + "_" + App.AnimsSubFolderName
	End Function
	...
	Public Function GetAnimationSmdRelativePathFileName(ByVal anAnimationDesc As SourceMdlAnimationDesc) As String
		...
		'animationSmdRelativePathFileName = Path.Combine(Me.GetAnimationSmdRelativePath, anAnimationDesc.theName)
		'======
		animationSmdRelativePathFileName = Me.GetAnimationSmdRelativePath
		animationSmdRelativePathFileName += Path.DirectorySeparatorChar
		If anAnimationDesc.theName(0) = "@"c Then
			animationSmdRelativePathFileName += anAnimationDesc.theName.Substring(1)
		Else
			animationSmdRelativePathFileName += anAnimationDesc.theName
		End If
	...
	Public thePhyFileHeader As SourcePhyFileHeader
	Public theMdlFileHeader As SourceMdlFileHeader
	Public theAniFileHeader As SourceAniFileHeader
	Public theVtxFileHeader As SourceVtxFileHeader
	Public theVvdFileHeader As SourceVvdFileHeader
