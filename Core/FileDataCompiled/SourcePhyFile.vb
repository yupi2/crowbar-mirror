...
#Region "Methods"
	...
	Public Sub ReadFile(ByVal inputPathFileName As String, ByVal aSourceEngineModel As SourceModel)
		...
		'inputPathFileName = Path.ChangeExtension(pathFileName, ".phy")
		...
		Try
			...
			If inputFileStream IsNot Nothing Then
				Try
					...
					Me.ReadSourcePhyHeader()
					If Me.theSourceEngineModel.thePhyFileHeader.solidCount > 0 Then
						Me.ReadSourceCollisionData()
						Me.CalculateVertexNormals()
						Me.ReadSourcePhysCollisionModels()
						Me.ReadSourcePhyRagdollConstraintDescs()
						Me.ReadSourcePhyCollisionRules()
						Me.ReadSourcePhyEditParamsSection()
						Me.ReadCollisionTextSection()

						''NOTE: ReadStudioVertexes() must be after ReadStudioBodyParts(), because of the needed vertex count?
						'If Me.theHeader.vertexBaseP > 0 Then
...
#Region "Private Methods"
	...
	Private Sub ReadSourcePhyHeader()
		Me.theSourceEngineModel.thePhyFileHeader = New SourcePhyFileHeader()
		...
		'12 00 00 00 
		'1f de 9d 20 
		Me.theSourceEngineModel.thePhyFileHeader.size = Me.theInputFileReader.ReadInt32()
		Me.theSourceEngineModel.thePhyFileHeader.id = Me.theInputFileReader.ReadInt32()
		Me.theSourceEngineModel.thePhyFileHeader.solidCount = Me.theInputFileReader.ReadInt32()
		Me.theSourceEngineModel.thePhyFileHeader.checksum = Me.theInputFileReader.ReadInt32()

		'NOTE: If header size ever increases, this will at least skip over extra stuff.
		Me.theInputFileReader.BaseStream.Seek(Me.theSourceEngineModel.thePhyFileHeader.size, SeekOrigin.Begin)
	End Sub
	...
	Private Sub ReadSourceCollisionData()
		...
		Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyCollisionDatas = New List(Of SourcePhyCollisionData)()
		For solidIndex As Integer = 0 To Me.theSourceEngineModel.thePhyFileHeader.solidCount - 1
			...
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyCollisionDatas.Add(collisionData)

			Me.theInputFileReader.BaseStream.Seek(nextSolidDataStreamPosition, SeekOrigin.Begin)
		Next
		Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyKeyValueDataOffset = Me.theInputFileReader.BaseStream.Position
	End Sub
	...
	Private Sub ReadSourcePhysCollisionModels()
		Try
			Dim line As String
			...
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyPhysCollisionModels = New List(Of SourcePhyPhysCollisionModel)()
			...
			Do
				...
				If key Is Nothing OrElse key <> "}" Then
					Exit Do
				End If
				Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyPhysCollisionModels.Add(aSourcePhysCollisionModel)
				thereIsAValue = True
			...
			maxCount = 0
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues = New SourcePhyPhysCollisionModel()
			For i As Integer = 0 To Me.theDampingToCountMap.Count - 1
				...
				End If
			Next
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues.theDamping = maxValue
			maxCount = 0
			For i As Integer = 0 To Me.theInertiaToCountMap.Count - 1
				...
				End If
			Next
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues.theInertia = maxValue
			maxCount = 0
			For i As Integer = 0 To Me.theRotDampingToCountMap.Count - 1
				...
				End If
			Next
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyPhysCollisionModelMostUsedValues.theRotDamping = maxValue
		Catch
			Throw
			'Finally
		End Try
	...
	Private Sub ReadSourcePhyRagdollConstraintDescs()
		Try
			Dim line As String
			Dim thereIsAValue As Boolean = True
			Dim key As String = ""
			...
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyRagdollConstraintDescs = New SortedList(Of Integer, SourcePhyRagdollConstraint)()
			Do
				...
				If key Is Nothing OrElse key <> "}" Then
					Exit Do
				End If
				Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyRagdollConstraintDescs.Add(aSourceRagdollConstraintDesc.theChildIndex, aSourceRagdollConstraintDesc)
				thereIsAValue = True
			Loop Until line Is Nothing
		Catch
		Finally
	...
	Private Sub ReadSourcePhyCollisionRules()
		Try
			Dim line As String
			Dim thereIsAValue As Boolean = True
			Dim key As String = ""
			...
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyCollisionPairs = New List(Of SourcePhyCollisionPair)()
			Do
				...
				While thereIsAValue
					...
					If thereIsAValue Then
						...
						ElseIf key = "selfcollisions" Then
							Me.theSourceEngineModel.thePhyFileHeader.theSourcePhySelfCollides = False
						End If
					End If
				End While
				...
				If key Is Nothing OrElse key <> "}" Then
					Exit Do
				End If
				Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyCollisionPairs.Add(aSourcePhyCollisionPair)
				thereIsAValue = True
			Loop Until line Is Nothing
		Catch
		Finally
	...
	Private Sub ReadSourcePhyEditParamsSection()
		Try
			Dim line As String
			Dim thereIsAValue As Boolean = True
			Dim key As String = ""
			Dim value As String = ""
			Dim tempStreamOffset As Long
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection = New SourcePhyEditParamsSection()
			Do
				tempStreamOffset = Me.theInputFileReader.BaseStream.Position
				line = FileManager.ReadTextLine(Me.theInputFileReader)
				...
				While thereIsAValue
					...
					If key = "rootname" Then
						thereIsAValue = True
						If key <> value Then
							Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection.rootName = value
							'Else
							'	Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection.rootName = ""
						End If
					ElseIf thereIsAValue Then
						If key = "concave" Then
							Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection.concave = value
						ElseIf key = "totalmass" Then
							'Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection.totalMass = CSng(value)
							Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyEditParamsSection.totalMass = Single.Parse(value, TheApp.InternalNumberFormat)
						End If
					End If
				End While
	...
	Private Sub ReadCollisionTextSection()
		...
		Try
			...
			Me.theSourceEngineModel.thePhyFileHeader.theSourcePhyCollisionText = Me.theInputFileReader.ReadChars(CInt(streamLastPosition - Me.theInputFileReader.BaseStream.Position - 1))
		Catch
		Finally
		End Try
	End Sub

	Private Sub ReadStudioVertexes()
		'	Me.theInputFileReader.BaseStream.Seek(Me.theHeader.vertexBaseP, SeekOrigin.Begin)
		'	'Dim vertexInputFileStreamPosition As Long
		'	'Dim inputFileStreamPosition As Long
		'	Dim totalVertexeCount As Integer
		'	totalVertexeCount = Me.theBodyParts(0).theModels(0).vertexCount
		'	Me.theVertexes = New List(Of StudioVertex)(totalVertexeCount - 1)
		'	For i As Integer = 0 To totalVertexeCount - 1
		'		'vertexInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
		'		Dim aVertex As New StudioVertex()
		'		aVertex.boneWeightData = New StudioBoneWeightData()
		'		For x As Integer = 0 To MAX_NUM_BONES_PER_VERT - 1
		'			aVertex.boneWeightData.weight(x) = Me.theInputFileReader.ReadSingle()
		'		Next
		'		For x As Integer = 0 To MAX_NUM_BONES_PER_VERT - 1
		'			aVertex.boneWeightData.bone(x) = Me.theInputFileReader.ReadByte()
		'		Next
		'		aVertex.boneWeightData.boneCount = Me.theInputFileReader.ReadByte()
		'		aVertex.positionX = Me.theInputFileReader.ReadSingle()
		'		aVertex.positionY = Me.theInputFileReader.ReadSingle()
		'		aVertex.positionZ = Me.theInputFileReader.ReadSingle()
		'		aVertex.normalX = Me.theInputFileReader.ReadSingle()
		'		aVertex.normalY = Me.theInputFileReader.ReadSingle()
		'		aVertex.normalZ = Me.theInputFileReader.ReadSingle()
		'		aVertex.texCoordX = Me.theInputFileReader.ReadSingle()
		'		aVertex.texCoordY = Me.theInputFileReader.ReadSingle()
		'		Me.theVertexes.Add(aVertex)
